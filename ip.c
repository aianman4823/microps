...
int ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface))
{
  ...
  protocols = entry;
  infof("registered, tyep=%u", entry->type);
  return 0;
}
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
  ...
  if (hlen > len)
  {
    errorf("input data length too short");
    return;
  }
  ...
  for (proto = protocols; proto; proto = proto->next)
  {
    ...
    {
      proto->handler((uint8_t *)hdr + len, total - hlen, hdr->src, hdr->dst, iface);
      return;
    }
  }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
  ...
  else
  {
    ...
    if (!iface)
    {
      errorf("ip_iface_select(src) failure: addr=%s", ip_addr_ntop(src, addr, sizeof(addr)));
      return -1;
    }
