...
int net_device_register(struct net_device *dev)
{
    ...
    dev->next = devices;
    devices = dev;
    infof("registered, dev=%s, tyep=0x04x", dev->name, dev->type);
    return 0;
}
...
int net_device_input(uint16_t type, const uint8_t *data, size_t len, struct net_device *dev)
{
    debugf("dev=%s, type=0x%04x, len=%zu", dev->name, type, len);
    debugdump(data, len);
    return 0;
...
int net_softirq_handler(void)
{
    ...
    for (proto = protocols; proto; proto = proto->next)
    {
        while (1)
        {
            ...
            debugdump(entry->data, entry->len, entry->dev);
